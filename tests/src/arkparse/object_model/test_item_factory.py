"""Generating inventory items from the per-archetype templates.

Every test here writes to a save, so they run against a copy taken with
``store_db`` rather than against the shared fixture.
"""
...
def writable_save(rag_limited: AsaSave, temp_file_folder: Path) -> AsaSave:
    """A copy of the small Ragnarok save, safe to generate objects into."""
    copy = temp_file_folder / "item_generation.db"
    rag_limited.store_db(copy)
    assert copy.exists(), "Database copy should be created"
    return AsaSave(path=copy)
...
def test_archetype_is_derived_from_the_class(blueprint: str, expected: ArkItemArchetype):
    assert ItemFactory.archetype_of(blueprint) is expected


def test_engram_uses_the_blueprint_template():
    """An engram shares its class with the item but needs the _bp template."""
    ...
    engram = ItemFactory.archetype_of(Classes.equipment.weapons.advanced.longneck,
                                      is_blueprint=True)
    assert not item.is_blueprint
    assert engram is ArkItemArchetype.WEAPON_BP and engram.is_blueprint


# A generated item must come back out of the save as the class that was asked
# for, not as the class of the template it was stamped from.
...
# defined by: those can be written later, but never added.
...
def test_generated_inventory_starts_empty(writable_save: AsaSave):
...
def test_items_can_be_added_to_an_empty_inventory(writable_save: AsaSave):
    """The first item inserts the InventoryItems array; the rest replace it."""
    inventory = Inventory.generate_from_template(writable_save, STORAGE_BOX)
    assert inventory.number_of_items == 0
...
def test_item_added_to_inventory_points_back_at_it(writable_save: AsaSave):
    inventory = Inventory.generate_from_template(writable_save, STORAGE_BOX)
    item = ItemFactory.generate(writable_save, Classes.resources.Basic.metal,
                                owner_inventory_uuid=inventory.object.uuid)
    inventory.add_item(item.object.uuid)
    ...
    reread = InventoryItem(item.object.uuid, writable_save)
    assert reread.owner_inv_uuid == inventory.object.uuid
